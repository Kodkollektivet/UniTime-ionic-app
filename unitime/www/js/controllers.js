...
.controller('DashCtrl', function($scope, $interval, $ionicSlideBoxDelegate, $ionicScrollDelegate, RootData, Course, SingleCourse, $state) {
        ...
        $scope.getCourseInfo = function (course_code) {
            SingleCourse.get({course: course_code}, function (response) {
                ...
                RootData.setSlideIndex(index);
                $state.go('tab.dash-detail');
            })

        };

        $scope.goToAddCourses = function () {
            $ionicSlideBoxDelegate.$getByHandle('slideHandle').slide(0);
        };

        $scope.goToMyCourses = function () {
            $ionicSlideBoxDelegate.$getByHandle('slideHandle').slide(1);
        };
    })
            ...
            .controller('DashDetailCtrl', function ($scope, $http, $stateParams, RootData) {
                $scope.course_info = RootData.getSelectedCourse();
                $scope.index = RootData.getSlideIndex();
                $scope.courses = [];
                $scope.selected_courses = [];
